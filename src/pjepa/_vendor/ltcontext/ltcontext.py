...
from pjepa._vendor.ltcontext.multi_head_attention import MultiHeadAttention, AttentionParams
from pjepa._vendor.ltcontext.attention_utils import convert_to_patches
from pjepa._vendor.ltcontext.helpers import exists
...
class LTContextAttention(BaseAttention):
    ...
    def _reshape(self, x: Tensor, masks: Tensor = None):
        ...
        lt_patches = rearrange(
            patches, "b d num_patches patch_size -> (b patch_size) d num_patches"
        )
...
class LTC(nn.Module):
    def __init__(self, model_cfg):
        ...
        attn_params = AttentionParams(num_heads=attn_cfg.NUM_ATTN_HEADS, dropout=attn_cfg.DROPOUT)
