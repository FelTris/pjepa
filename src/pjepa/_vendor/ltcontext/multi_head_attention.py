...
from pjepa._vendor.ltcontext.attention_utils import ScaledDotProduct
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        if attention_params.use_separate_proj_weight:
            ...
            assert dim_key == dim_value, (
                "To share qkv projection weights dimension of q, k, v should be the same"
            )
            self.proj_q = LinearProj(dim_model, dim_key)
    ...
    def forward(
        ...
        z, attn_matrix = self.attention(
            q=q, k=k, v=v, att_mask=att_mask, return_attn_matrix=True, position_bias=position_bias
        )
